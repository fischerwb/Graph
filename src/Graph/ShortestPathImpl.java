...
public class ShortestPathImpl implements ShortestPath {
  private final List<VertexImpl> unsettled;
  private final List<VertexImpl> settled;
  ...
  private class PathAndDistance {
    ...
    PathAndDistance(double d, EdgeImpl e) {
      distance = d;
      edge = e;
    }
  }
   ...
   */
  void setPathAndDistance(String key, double distance, EdgeImpl edge) {
   ...
   */
  public ShortestPathImpl (Graph graph, VertexImpl from) {
    unsettled = new ArrayList<VertexImpl>(10);
    settled = new ArrayList<VertexImpl>(graph.getSize());
    ...
    while (!unsettled.isEmpty()) {
      VertexImpl evalVertex = getVertexWithShortestDistance();
   ...
   */
  private VertexImpl getVertexWithShortestDistance() {
    VertexImpl ret = null;
   ...
   */
  private void evaluateAdjacentVertices(VertexImpl evalVertex) {
    for (EdgeImpl e : evalVertex.outgoingEdges()) {
      VertexImpl to = e.to();
